...
class MedicineResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'expirationDate' => $this->when(Route::is('carts.*'),$this->pivot->expirationDate,$this->expirationDate),
            'isExpired' => $this->when(!$report && !$stat,$this->when($this->expirationDate > now(), false, true)),
            'price' => $this->when(Route::is('carts.*'),$this->pivot->price,$this->price) ,
            'profit' => $this->when(Route::is('admin.*'), $this->when(Route::is('carts.*'), $this->pivot->profit,$this->profit)),
